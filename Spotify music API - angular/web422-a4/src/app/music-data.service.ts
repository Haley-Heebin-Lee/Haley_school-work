...
export class MusicDataService {
  ...
  getNewReleases(): Observable<SpotifyApi.ListOfNewReleasesResponse> {
      return this.spotifyToken.getBearerToken().pipe(mergeMap((token:any)=>{
  ...
  removeFromFavourites(id:string): Observable<SpotifyApi.MultipleTracksResponse> {
    ...
    .pipe(mergeMap((favouritesArray:any) => {
    ...
    .pipe(mergeMap((favouritesArray:any) => {
      ...
      if (favouritesArray.length > 0) {
        return this.spotifyToken.getBearerToken().pipe(
          mergeMap((token:any) => {return this.http.get<any>(`https://api.spotify.com/v1/tracks?ids=${favouritesArray.join()}`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
